...
export async function computePersonalExpenseCharts(userId: string, startDate: Date, endDate: Date) {
  ...
  // --- 2. PIE CHART ---
  const categoryDataMap = new Map<string, { name: string; color: string; amount: number }>()
  ...
  const allPieData = Array.from(categoryDataMap.values()).sort((a, b) => b.amount - a.amount)
  ...
  if (allPieData.length <= 6) {
    ...
    pieChartData = [...top5, { name: 'Khác', color: '#cbd5e1', amount: restAmount }]
  }
